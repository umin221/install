let apiList = {
    ...
    return {
      ...
      data: {
        'body': {
          ...
          'ViewMode': 'Sales Rep',
          'SearchSpec': '(' + KND.Util.condition2D(condition, 'Order Entry - Orders', ' OR ', operator) + ') AND [Order Entry - Orders.Order Type] = LookupValue("FS_ORDER_TYPE", "Sparts Picking")',
  ...
  getProduct: option => {
    let model = 'AND ([Product.KL Translated Name] ~LIKE "*' + option.data.val + '*" OR [Product.Name] ~LIKE "*' + option.data.val + '*")';
    return {
      method: 'post',
